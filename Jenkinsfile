    ...
    stages {
        ...
        stage('API Test') {
            ...
            steps {
                ...
                sh 'newman run $FOLDER/Tests.postman_collection.json -g $FOLDER/workspace.postman_globals.json --env-var "baseUrl=$TOMCAT_SERVER:$PORT/$APP" --disable-unicode --reporters cli,html,openapi,postman-cloud,xunit --reporter-html-export target/pipelineReport.html --reporter-openapi-spec postman/schemas/index.yaml --reporter-postman-cloud-apiKey "${POSTMAN_API_KEY}" --reporter-postman-cloud-workspaceId ${WORKSPACE_ID} --reporter-integrationIdentifier "${WORKSPACE_ID}-${JOB_NAME}${BUILD_NUMBER}"'                
            }       
        }   
    }
post { 
